...
class Sequence(object):
    ...
    Every `Sequence` must implements the `__getitem__` and the `__len__`
    methods. If you want to modify your dataset between epochs you may
    implement `on_epoch_end`.
...
def random_zoom(x, zoom_range, row_axis=1, col_axis=2, channel_axis=0,
    ...
    if len(zoom_range) != 2:
        raise ValueError('`zoom_range` should be a tuple or list of two \
                         floats. ''Received arg: ', zoom_range)
...
def random_channel_shift(x, intensity, channel_axis=0):
    ...
    channel_images = [np.clip(x_channel
                      + np.random.uniform(-intensity, intensity), min_x, max_x)
                      for x_channel in x]
        ...
        if self.save_to_dir:
            for i, j in enumerate(index_array):
                ...
                fname = '{prefix}_{index}_{hash}.{format}'.format(
                    prefix=self.save_prefix, index=j,
...
def _count_valid_files_in_directory(directory, white_list_formats,
    ...
    # Arguments
        directory: absolute path to the directory containing files to be
        counted white_list_formats: set of strings containing allowed
        extensions for the files to be counted.
        follow_links: boolean.
...
class ImageDataGenerator(object):
    ...
    def random_transform(self, x, seed=None):
        ...
        if self.shear_range:
            shear = np.deg2rad(np.random.uniform(-self.shear_range,
                                                 self.shear_range))
        else:
            shear = 0
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        # second, build an index of the images of different class subfolders
        results = []
...
def _doubledir_list_valid_filenames_in_directory(directory, white_list_formats,
                                                 class_indices, follow_links):
    """List paths of files in `subdir` with extensions in `white_list_formats`.
    ...
    for root, _, files in _recursive_list(directory):
        for fname in sorted(files):
            ...
            if is_valid:
                ...
                # add filename relative to directory
                absolute_path = os.path.join(root, fname)
                filenames.append(os.path.join(directory, absolute_path))
    return classes, filenames


class DoubleDirImageDataGenerator(object):
    """Generate minibatches of image data with real-time data augmentation.
    ...
    def __init__(self,
        ...
        if data_format not in {'channels_last', 'channels_first'}:
            raise ValueError('`data_format` should be `"channels_last"` '
                             '(channel after row and '
                             'column) or `"channels_first"` (channel before '
                             'row and column). '
                             'Received arg: ', data_format)
        self.data_format = data_format
        if data_format == 'channels_first':
    ...
    def flow_from_directory(self, directory,
                            ...
                            follow_links=False,
                            interpolation='nearest'):
        return DoubleDirDirectoryIterator(
            directory, self,
            target_size=target_size, color_mode=color_mode,
            classes=classes, class_mode=class_mode,
            data_format=self.data_format,
...
class DoubleDirDirectoryIterator(Iterator):
    """Iterator capable of reading images from a directory on disk.
    ...
    def __init__(self, directory, image_data_generator,
        ...
        if not classes:
            classes = []
            for subdir in sorted(os.listdir(directory[0])):
                if os.path.isdir(os.path.join(directory[0], subdir)):
                    classes.append(subdir)
        self.num_classes = len(classes)
        self.class_indices = dict(zip(classes, range(len(classes))))

        pool = multiprocessing.pool.ThreadPool()
        for dir_i in range(np.shape(directory)[0]):
            function_partial = partial(_count_valid_files_in_directory,
                                       white_list_formats=white_list_formats,
                                       follow_links=follow_links)
            self.samples = self.samples+sum(
                pool.map(function_partial, (
                    os.path.join(directory[dir_i], subdir)
                         for subdir in classes)))
        ...
        self.filenames = []
        self.classes = np.zeros((int(self.samples),), dtype='int32')
        i = 0
        for dir_i in range(np.shape(directory)[0]):
            for dirpath in (os.path.join(directory[dir_i],
                                         subdir) for subdir in classes):
                results.append(pool.apply_async(
                    _doubledir_list_valid_filenames_in_directory,
                    (dirpath, white_list_formats,
        ...
        pool.close()
        pool.join()
        super(DoubleDirDirectoryIterator, self).__init__(
...
def _DAdir_list_valid_filenames_in_directory(directory, white_list_formats,
                                             class_indices, follow_links,
                                             cur_directory, directory_list):
    """List paths of files in `subdir` with extensions in `white_list_formats`.
    ...
    for root, _, files in _recursive_list(directory):
        for fname in sorted(files):
            ...
            if is_valid:
                ...
                absolute_path = os.path.join(root, fname)
                filenames.append(os.path.join(directory, absolute_path))
                domains.append(directory_list.index(cur_directory))
    return classes, filenames, domains


class DAImageDataGenerator(object):
    """Generate minibatches of image data with real-time data augmentation.
...
class DADirectoryIterator(Iterator):
    """Iterator capable of reading images from a directory on disk.
    ...
    def __init__(self, directory, image_data_generator,
        ...
        self.num_classes = len(classes)
        self.class_indices = dict(zip(classes, range(len(classes))))
        self.num_domains = len(directory)
        pool = multiprocessing.pool.ThreadPool()
        for dir_i in range(np.shape(directory)[0]):
            function_partial = partial(_count_valid_files_in_directory,
                                       white_list_formats=white_list_formats,
        ...
        print('Found %d images belonging to %d classes and %d domains.'
              % (self.samples, self.num_classes, self.num_domains))

        results = []

        self.filenames = []
        self.classes = np.zeros((int(self.samples),), dtype='int32')
        self.domains = np.zeros((int(self.samples),), dtype='int32')
        i = 0
        for dir_i in range(np.shape(directory)[0]):
            for dirpath in (os.path.join(directory[dir_i], subdir)
                            for subdir in classes):
                results.append(pool.apply_async(
                    _DAdir_list_valid_filenames_in_directory,
                    (dirpath, white_list_formats,
                     self.class_indices, follow_links, directory[dir_i],
                     self.directory)))
        for res in results:
            classes, filenames, domains = res.get()
            self.classes[i:i + len(classes)] = classes
            self.domains[i:i + len(classes)] = domains
            self.filenames += filenames
            i += len(classes)
        pool.close()
        pool.join()
        super(DADirectoryIterator, self).__init__(
            self.samples, batch_size, shuffle, seed)
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        if self.domain_mode == 'binary':
            batch_d = self.domains[index_array].astype(K.floatx())
        elif self.domain_mode == 'categorical':
            batch_d = np.zeros((len(batch_x), len(self.directory)),
                               dtype=K.floatx())
            for i, label in enumerate(self.domains[index_array]):
                batch_d[i, label] = 1.
